...
export default function Reports() {
  ...
  return (
    <div className="space-y-6 sm:space-y-8 pb-12">
      <div>
        <h1 className="text-2xl sm:text-3xl lg:text-4xl font-display font-bold text-white tracking-widest uppercase">Analytics Matrix</h1>
        <p className="text-muted-foreground font-mono mt-1 sm:mt-2 text-sm">Deep data visualisation</p>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5 sm:gap-8">
        <Card className="glass-panel border-white/10 lg:col-span-2">
          <CardHeader className="pb-2">
            <CardTitle className="font-display tracking-widest text-base sm:text-lg text-primary text-glow-cyan">Net Asset Growth (6mo)</CardTitle>
          </CardHeader>
          <CardContent className="h-[250px] sm:h-[350px] lg:h-[400px]">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={historyData} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
                ...
                <XAxis dataKey="month" stroke="#4a7a9b" fontFamily="Space Mono" fontSize={11} tickLine={false} axisLine={false} />
                <YAxis stroke="#4a7a9b" fontFamily="Space Mono" fontSize={10} tickLine={false} axisLine={false} tickFormatter={(val) => '₹' + (val >= 1000 ? (val / 1000).toFixed(0) + 'k' : val)} width={45} />
                <RechartsTooltip
        ...
        <Card className="glass-panel border-white/10">
          <CardHeader className="pb-2">
            <CardTitle className="font-display tracking-widest text-base sm:text-lg text-accent text-glow-blue">Actual Spending by Category</CardTitle>
          </CardHeader>
          <CardContent className="h-[280px] sm:h-[350px]">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={categoryData} layout="vertical" margin={{ top: 5, right: 20, left: 0, bottom: 5 }}>
                ...
                <YAxis dataKey="name" type="category" stroke="#e8f4ff" fontFamily="Syne" fontSize={11} tickLine={false} axisLine={false} width={80} />
                <RechartsTooltip
                  cursor={{ fill: 'rgba(255,255,255,0.05)' }}
                  formatter={(value: number) => [formatINR(value), '']}
                  contentStyle={{ backgroundColor: 'rgba(7, 21, 37, 0.9)', border: '1px solid rgba(0, 144, 255, 0.3)', borderRadius: '8px' }}
                />
                <Bar dataKey="value" fill="#0090ff" radius={[0, 4, 4, 0]} barSize={18} />
              </BarChart>
            </ResponsiveContainer>
          </CardContent>
        </Card>
        ...
        <Card className="glass-panel border-white/10 bg-gradient-to-br from-black/80 to-primary/10">
          <CardHeader className="pb-2">
            <CardTitle className="font-display tracking-widest text-base sm:text-lg">System Insights</CardTitle>
          </CardHeader>
